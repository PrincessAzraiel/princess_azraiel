...
export default function LandingPage() {
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden bg-[#030303] text-pink-50 selection:bg-pink-500 selection:text-black">
      ...
      <div className="fixed inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 pointer-events-none z-10 brightness-150 contrast-150 mix-blend-overlay" />
      ...
      <main className="relative z-20 min-h-screen flex items-center justify-center p-4 md:p-8 lg:p-16">
        ...
        <div className="w-full max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 lg:gap-16 items-start">
          ...
          <section className="w-full lg:w-[35%] lg:sticky lg:top-24 flex flex-col items-center lg:items-start text-center lg:text-left space-y-8">
            ...
            {/* Holographic Panel Container - Now Reactive */}
            <SpotlightCard className="w-full rounded-2xl bg-white/[0.02] border border-white/10 backdrop-blur-xl shadow-2xl overflow-hidden group">
              <div className="relative z-10 p-8 md:p-10">
                ...
                {/* Status Badge */}
                <div className="inline-flex items-center gap-3 border border-pink-500/30 bg-pink-950/30 px-4 py-1.5 rounded-full mb-8 shadow-[0_0_15px_rgba(236,72,153,0.2)]">
                ...
                <div className="space-y-2 mb-8 relative z-10">
                  <h1 className="font-italiana text-6xl md:text-7xl leading-[0.85] text-transparent bg-clip-text bg-gradient-to-b from-white via-pink-100 to-pink-950 drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]">
                    Princess <br/> Azraiel
                  </h1>
                  <div className="h-px w-24 bg-gradient-to-r from-pink-500 to-transparent mx-auto lg:mx-0 my-4" />
                  <p className="font-syncopate text-[10px] md:text-xs uppercase tracking-[0.3em] text-pink-400 font-bold">
                    Corruption Hub // V4.1
                  </p>
                </div>

                {/* Quote */}
                <p className="font-manrope text-white/70 text-sm md:text-base italic leading-relaxed mb-8">
                  "Submit. Suffer. Smile.<br/> Your digital devotion is required."
                </p>

                {/* Social Icons Grid */}
                <div className="grid grid-cols-5 gap-2 pt-6 border-t border-white/5 relative z-20">
            ...
            <p className="hidden lg:block font-manrope text-xs text-white/20 pl-4 border-l border-white/10">
...
// This component tracks mouse position to create the dynamic glow effect
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          opacity,
          background: `radial-gradient(400px circle at ${position.x}px ${position.y}px, rgba(236, 72, 153, 0.4), transparent 40%)`, // Higher intensity for border
          maskImage: `linear-gradient(black, black) content-box, linear-gradient(black, black)`,
          maskComposite: `exclude`, // This ensures only the border glows
...
function LinkCard({ 
  ...
  // Visual Variants (Backgrounds are now handled by SpotlightCard mostly, but we keep base tints)
  const variants = {
    primary: "bg-gradient-to-br from-pink-950/40 to-black/80 border border-pink-500/40",
    glass: "bg-white/[0.03] backdrop-blur-md border border-white/10 hover:bg-white/[0.05]",
    glow: "bg-black/80 animate-border-flow border border-pink-500",
    ghost: "bg-transparent border border-transparent hover:bg-white/5 hover:border-white/10 opacity-70 hover:opacity-100"
  };
    ...
    <Link href={href} target={isExternal ? "_blank" : undefined} rel={isExternal ? "noopener noreferrer" : undefined} className="block w-full h-full perspective-1000">
      <SpotlightCard className={`h-full rounded-xl ${variants[variant]}`}>
        <div className={baseStyles}>
          ...
          {/* Animated Background Gradient for Primary/Glow */}
          {(variant === 'primary' || variant === 'glow') && (
             <div className="absolute inset-0 bg-gradient-to-r from-transparent via-pink-500/10 to-transparent -skew-x-12 translate-x-[-200%] group-hover:animate-[shimmer_1.5s_infinite] pointer-events-none" />
          )}
          ...
          <div className="flex justify-between items-start mb-4 relative z-10">
            <div className={`
              ...
              ${variant === 'primary' || variant === 'glow' ? 'bg-pink-500 text-black shadow-[0_0_15px_rgba(236,72,153,0.6)]' : 'bg-white/10 text-pink-200 group-hover:bg-pink-500 group-hover:text-black group-hover:shadow-[0_0_15px_rgba(236,72,153,0.4)]'}
            `}>
              {icon}
            </div>
          ...
          <div className="relative z-10">
            <h3 className={`font-italiana leading-none mb-1 group-hover:text-pink-100 transition-colors ${isHero ? 'text-4xl' : 'text-2xl'}`}>
              {label}
            </h3>
            {sub && (
              <p className="font-manrope text-[10px] md:text-xs text-white/40 uppercase tracking-widest group-hover:text-pink-300 transition-colors">
                {sub}
              </p>
            )}
          </div>
...
function SocialBtn({ href, icon }: { href: string; icon: React.ReactNode }) {
  return (
    <a 
      ...
      className="group flex items-center justify-center w-full aspect-square bg-white/5 border border-white/5 hover:border-pink-500 hover:bg-pink-500/10 hover:shadow-[0_0_15px_rgba(236,72,153,0.3)] transition-all duration-300 rounded-lg relative overflow-hidden"
    >
      <div className="absolute inset-0 bg-pink-500/20 blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
